...
export function List() {
  ...
  const putPerson = async () => {
    await axios.put(baseUrl + "/" + currentUser.id, currentUser)
      .then(response => {
        ...
        updatedData.map(ppl => {
          if (ppl.id === currentUser.id) {
            ppl.firstName = result.firstName;
            ppl.lastName = result.lastName;
            ppl.dateOfBirth = result.dateOfBirth;
            ppl.sex = result.sex;
          }
        });
        GetPerson();
  ...
  const deletePerson = async () => {
    await axios.delete(baseUrl + "/" + currentUser.id)
      .then(() => {
        setData(data.filter(ppl => ppl.id !== currentUser.id));
        openCloseModalDelete();
      }).catch(error => {
        console.log(error);
      })
        ...
        <tbody>
          {data.map(ppl => (
            <tr key={ppl.id}>
              ...
              <td>{ppl.firstName}</td>
              <td>{ppl.lastName}</td>
              <td>{ppl.dateOfBirth}</td>
              <td>{ppl.sex}</td>
              <td>
                <Button variant="outline-primary" onClick={() => selectCurrentUser(ppl, "Edit")}>Edit</Button>{"  "}
                <Button variant="outline-warning" onClick={() => selectCurrentUser(ppl, "Details")}>Details</Button>{"  "}
                <Button variant="outline-danger" onClick={() => selectCurrentUser(ppl, "Delete")}>Delete</Button>
              </td>
            </tr>
          ))}
        </tbody>
        ...
        <ModalBody>
          <Form>
            ...
            <Form.Group>
              ...
              <Form.Control type="text" id="txtLastName" name="lastName" placeholder="Clavijo Dominguez" required onChange={handleChange} />
            </Form.Group>
            <Form.Group>
